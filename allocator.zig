...
const ZigCAllocator = struct {
    ...
    const MetaData = extern struct {
        ...
        pub fn init(alloc_size: usize) MetaData {
            return .{
                .size = alloc_size,
            };
        }
    ...
    inline fn addHeader(ptr: [*]u8, size: usize) void {
        const header: *MetaData = @ptrCast(@alignCast(ptr));
        header.* = MetaData.init(size);
    }
    ...
    fn alloc(self: ZigCAllocator, comptime alignment: ?u29, size: usize) ?*anyopaque {
        ...
        const aligned_ptr = self.backing_allocator.alignedAlloc(u8, alignment orelse .fromByteUnits(max_align_t), full_size) catch return null;
        addHeader(aligned_ptr.ptr, size);
    ...
    fn free(self: ZigCAllocator, ptr: ?*anyopaque) void {
        if (ptr) |p| {
            ...
            const full_slice: []align(max_align_t) u8 = @alignCast(@as([*]u8, @ptrCast(header))[0 .. header.size + @sizeOf(MetaData)]);
    ...
    fn realloc(self: ZigCAllocator, ptr: ?*anyopaque, new_size: usize) ?*anyopaque {
        ...
        const full_old: []align(max_align_t) u8 = @alignCast(@as([*]u8, @ptrCast(header))[0 .. old_size + @sizeOf(MetaData)]);
        const full_new = self.backing_allocator.realloc(full_old, new_size + @sizeOf(MetaData)) catch return null;

        addHeader(full_new.ptr, new_size);
